...
/**
 * @zk_title   		Панель: настройки
 * @zk_routable		0
 */
class Zkernel_Controller_Cconf extends Zkernel_Controller_Action {
	function ctlinit() {
    	...
    	switch ($request->getParam('step')) {
    		case 'select':
    			...
    			if ($cnr) {
			    	...
			    	if (!class_exists($c)) include $dir.'/'.$c.'.php';
    				if (class_exists($c)) {
			    	...
			    	if (@$met) {
			    		foreach ($met as $el) {
			    			...
			    			if (substr($el->name, 0, 3) == 'ctl' && substr($el->name, strlen($el->name) - 6) == 'Action') {
			    				...
			    				if (isset($db['zk_title'])) $k = $db['zk_title'].' ('.$k.') ';
			    				if ($k) $tt[$k] = $k;
			    			}
			    		}
			    	}
    		...
    		case 'field':
		    	...
		    	if ($c) {
			    	$hc = new Zkernel_Controller_Action_Helper_Control();
			    	$hc->init('', $c);
			    	$hc->configFromDb();
			    	$tt = array();
