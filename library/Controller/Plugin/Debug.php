...
/**
 * @see Zkernel_Controller_Plugin_Debug_Plugin_Text
 */
require_once 'Zkernel/Controller/Plugin/Debug/Plugin/Text.php';
 ...
 * @license    http://code.google.com/p/Zkernel/wiki/License     New BSD License
 */
class Zkernel_Controller_Plugin_Debug extends Zend_Controller_Plugin_Abstract
{
    /**
    ...
    public function __construct($options = null)
    {
         ...
         */
        $version = new Zkernel_Controller_Plugin_Debug_Plugin_Text();
     ...
     *
     * @param array $options
     * @return Zkernel_Controller_Plugin_Debug
     */
     ...
     *
     * @param Zkernel_Controller_Plugin_Debug_Plugin_Interface
     * @return Zkernel_Controller_Plugin_Debug
     */
    public function registerPlugin(Zkernel_Controller_Plugin_Debug_Plugin_Interface $plugin)
    {
     ...
     * @return Zkernel_Controller_Plugin_Debug_Plugin_Interface
     */
    ...
    public function dispatchLoopShutdown()
    {
        ...
        foreach ($this->_plugins as $plugin)
        {
            ...
            /* @var $plugin Zkernel_Controller_Plugin_Debug_Plugin_Interface */
            $html .= '<div id="Zkernel_' . $plugin->getIdentifier()
                  . '" class="Zkernel_panel">' . $panel . '</div>';
        }

        $html .= '<div id="Zkernel_info">';
        ...
        foreach ($this->_plugins as $plugin)
        {
            ...
            /* @var $plugin Zkernel_Controller_Plugin_Debug_Plugin_Interface */
            $html .= '<span class="Zkernel_span clickable" onclick="ZkernelPanel(\'Zkernel_' . $plugin->getIdentifier() . '\');">';
        ...
        $html .= '<span class="Zkernel_span Zkernel_last clickable" id="Zkernel_toggler" onclick="ZkernelSlideBar()">&#171;</span>';
    ...
    protected function _loadPlugins()
    {
    	foreach($this->_options['plugins'] as $plugin => $options) {
    	    ...
    	    if (in_array($plugin, Zkernel_Controller_Plugin_Debug::$standardPlugins)) {
    	        // standard plugin
                $pluginClass = 'Zkernel_Controller_Plugin_Debug_Plugin_' . $plugin;
    	    } else {
    	        // we use a custom plugin
                if (!preg_match('~^[\w]+$~D', $plugin)) {
                    throw new Zend_Exception("Zkernel: Invalid plugin name [$plugin]");
                }
                $pluginClass = $plugin;
            }
