...
class Zkernel_Video_Preview {
	...
	function create($name, $param = array()) {
		...
		if (@$param['time']) {
			...
			@exec('ffmpeg -i "'.$this->image_path.'/'.$name.'" '.($width && $height ? '-s '.$width.'x'.$height : '').' -deinterlace -r 25 -b 1000k -y -f flv -acodec libmp3lame -ac 1 -ar 44100 "'.$this->path.'/'.$prefix.$name.'"');
			if (!file_exists($this->path.'/'.$prefix.$name)) @exec('ffmpeg -i "'.$this->image_path.'/'.$name.'" '.($width && $height ? '-s '.$width.'x'.$height : '').' -deinterlace -r 25 -b 1000k -y -f flv -ac 1 -ar 44100 "'.$this->path.'/'.$prefix.$name.'"');
		}
