...
class Zkernel_View_Helper_Zlist extends Zend_View_Helper_Abstract {
    ...
    public function zlist($data = array()) {
        ...
        if ($data['fetch_model'] && !method_exists($data['fetch_model'], $data['fetch_method']))
            $data['fetch_method'] = 'fetch' . ucfirst($data['fetch_method']);

        $data['override_type'] = @$data['override_type'];
        if (!$data['override_type'])
            $data['override_type'] = strtolower(str_ireplace('Default_Model_', '', $data['fetch_model']));

        $data['view_script'] = @$data['view_script'];
        if (!$data['view_script'])
            $data['view_script'] = strtolower(str_ireplace('Default_Model_', '', $data['fetch_model'])) . '/' . strtolower(str_ireplace('fetch', '', $data['fetch_method']));
        if (stripos($data['view_script'], '/') === false)
            $data['view_script'] .= '/index';
        if (stripos($data['view_script'], '.phtml') === false)
            $data['view_script'] .= '.phtml';

        $data['view_empty'] = isset($data['view_empty']) ? $data['view_empty'] : true;

        if (!@$data['pager'] && (@$data['pager_url'] || @$data['pager_page'] || @$data['pager_perpage'] || @$data['pager_style'] || @$data['pager_script'] || @$data['pager_param']))
            $data['pager'] = true;

        $data['pager_url'] = @$data['pager_url'];
        if (!$data['pager_url'])
            $data['pager_url'] = ($reg ? '/' . $reg->stitle : '') . (is_object($data['fetch_model']) ? '' : '/' . strtolower(str_ireplace('Default_Model_', '', $data['fetch_model'])));

        $data['pager_page'] = @$data['pager_page'];
        if (!$data['pager_page'])
            $data['pager_page'] = @$this->view->page ? $this->view->page : 1;

        $data['pager_perpage'] = @$data['pager_perpage'];
        if (!$data['pager_perpage'])
            $data['pager_perpage'] = @$this->view->perpage ? $this->view->perpage : 10;

        $data['pager_style'] = @$data['pager_style'] ? $data['pager_style'] : 'All';

        $data['pager_script'] = @$data['pager_script'] ? $data['pager_script'] : 'pager';
        if (stripos($data['pager_script'], '/') === false)
            $data['pager_script'] .= '/index';
        if (stripos($data['pager_script'], '.phtml') === false)
            $data['pager_script'] .= '.phtml';
        ...
        if ($list instanceOf Zend_Db_Select) {
            if (@$data['pager'])
                ...
                $list = $class->getAdapter()->fetchAll($list);
        }
        if (!$lv && count($list)) {
        ...
        if (@$data['pager']) {
            ...
            if ($data['pager_page'] <= $paginator->count()) {
                $this->view->data = $list instanceOf Zend_Db_Select ? $this->view->override($paginator, $data['override_type']) : $paginator;
                $this->view->pager_count = $paginator->getTotalItemCount();
                $pager_param = $this->view->getVars();
                $pager_param['url'] = $data['pager_url'];
                if ($data['pager_param'])
                    $pager_param = array_merge($pager_param, $data['pager_param']);
                $this->view->pager = $this->view->paginationControl(
                        $paginator, $data['pager_style'], $data['pager_script'], $pager_param
                );
            }
        }
        $ret = '';
        if ($this->view->data || $data['view_empty']) {
            if (@$data['view_param'])
                foreach ($data['view_param'] as $k => $v)
                    $this->view->$k = $v;
