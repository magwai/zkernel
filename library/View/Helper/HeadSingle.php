...
class Zkernel_View_Helper_HeadSingle extends Zend_View_Helper_HeadScript  {
	function headSingle($name = null) {
		...
		foreach ($this as $item) {
        	...
        	if (stripos($item->attributes['src'], 'maps.google.com') !== false || stripos($item->attributes['src'], 'maps.googleapis.com') !== false || stripos($item->attributes['src'], 'api-maps.yandex.ru') !== false || preg_match('/^http\:\/\//i', $item->attributes['src'])) {
