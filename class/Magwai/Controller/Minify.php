...
class Magwai_Controller_Minify extends Zend_Controller_Action {
				...
				if ($res) {
					...
					if ($res_1) $res = $res_1;
						...
						if ($ret) {
							...
							$process = proc_open('java -client -Xmx64m -jar "'.SITE_PATH.'/lib/java/yuicompressor.jar" --charset utf-8 --type '.$ext, $desc, $pipes);
							if (is_resource($process)) {
								fwrite($pipes[0], $res);
								fclose($pipes[0]);
